...
#data imputation
max.vals <- apply(data,2,function(x) names(which.max(table(x))))
imput.data <- data
for ( i in 1:dim(data)[1]){
  for (j in 1:dim(data)[2]){
    if(is.na(data[i,j])){
      imput.data[i,j] <-   max.vals[j]
    }        
  }
}
...
### Structural_EM with Random Structure
...
for(i in 1: N_folds){
  ...
  test <- imput.data[select,]
  observed <- test$NM.Hazard
  test[,12:20] <- NA 
  train <- data[-select,]
  blclist <- matrix( c("Cytotoxicity", "NM.Hazard",    "Neurological.effects",  "NM.Hazard",  "Pulmonary.effects",  "NM.Hazard", "Fibrosis", 
                       "NM.Hazard",  "RCNS.effects",  "NM.Hazard", "Immunological.effects",  "NM.Hazard", "Genotoxicity",  "NM.Hazard",           
                       "Inflammation" ,   "NM.Hazard"  ), ncol = 2, byrow = TRUE)
  learned <-bnlearn::structural.em(train, maximize = "tabu", maximize.args=list(score= "bic", blacklist = blclist), 
                                    fit = "bayes",   impute = "bayes-lw", impute.args = list(n=2000), return.all = TRUE)
  ...
  for (j in 1:N_test){
    if(i == 10 && j >=56){
      ...
      pred[j] <- as.character(predict(training.fit, data=test[j,!is.na(test[j,])], 
                                      ...
                                      node= "NM.Hazard",prob = T, method = "parents", n=40000))
      if(is.na(pred[j])){
        pred[j] <- "NA"
      }
    }
